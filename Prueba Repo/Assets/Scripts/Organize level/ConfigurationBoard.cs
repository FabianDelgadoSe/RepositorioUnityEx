...
public class ConfigurationBoard : Photon.PunBehaviour {
    ...
    public void changeColorBoardSquares()
    {
        ...
        for (int i = 0; i<_boardSquaresArray.Length; i++)
        {
            ...
            _boardSquaresArray[i].GetComponent<Square>().photonView.RPC("changeSprite", PhotonTargets.AllBufferedViaServer,_codeColor);
    ...
    {
        int _quantityWalls = Random.RandomRange(3, 6);
        ...
        while (_quantityWalls > 0)
        {
            ...
            {
                _boardSquaresArray[_indexBoardSquare].GetComponent<Square>().photonView.RPC("changeSprite", PhotonTargets.AllBufferedViaServer, 5);
                _quantityWalls--;
